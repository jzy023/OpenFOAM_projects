...
Foam::label Foam::myEddy::Gamma2Values[] = {1, 2, 3, 4, 5, 6, 7, 8};
Foam::UList<Foam::label> Foam::myEddy::Gamma2(&Gamma2Values[0], 8);
int Foam::myEddy::debug = 0;

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

bool Foam::myEddy::setScales
(
    const scalar sigmaX,
    const label gamma2,
    const vector& e,
    ...
    vector& sigma,
    vector& alpha,
    // correction factor for larger computational domain size
    const scalar crr
) const
{
    ...
    for (label beta = 0; beta < vector::nComponents; ++beta)
    {
        ...
        else
        {
            ...
            // Yan 18.04.2020 -> ABL H = 1e3m
            // crr = 7.5e-4
            alpha[beta] = e[beta]*sqrt(crr*x/(2*c2));
        }
    }
    ...
    dir1_(0)
{
    ...
    // myEddy rotation from principal-to-global axes
    ...
    // Set the myEddy orientation to position of max eigenvalue
    // (direction of myEddy major axis, sigma_x in reference)
    dir1_ = 2;
    ...
    forAll(Gamma2, i)
    {
        ...
        if (setScales(sigmaX, gamma2, e, lambda, sigma_, alpha_, crr))
        {
            found = true;
            break;
        }
    ...
    else
    {
        if (debug)
        {
            ...
            WarningInFunction
                << "Unable to set myEddy intensity for myEddy: " << *this
                << endl;
        }

        // Remove the influence of this myEddy/indicate that its initialisation
        // failed
        patchFaceI_ = -1;
    }
}
...
Foam::myEddy::myEddy(const myEddy& e)
:
...
Foam::vector Foam::myEddy::uPrime(const point& xp, const vector& n) const
{
    // Relative position inside myEddy (global system) (PCR:p. 524)
    ...
    // Relative position inside myEddy (myEddy principal system)
    const vector rp(Rpg_.T() & r);

    // Shape function (myEddy principal system)
    const vector q(cmptMultiply(sigma_, vector::one - cmptMultiply(rp, rp)));

    // Fluctuating velocity (myEddy principal system) (PCR:Eq. 8)
...
void Foam::myEddy::writeCentreOBJ
(
    const vector& n,
    Ostream& os
) const
...
Foam::label Foam::myEddy::writeSurfaceOBJ
(
